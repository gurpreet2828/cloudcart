resource "null_resource" "kube_state_metrics" {
  ...
  provisioner "local-exec" {
   ...
   scp -o StrictHostKeyChecking=no -i ${var.ssh_key_private} -r ${path.root}/deploy-sock-shop/monitoring-app/kube-state-metrics ubuntu@${var.k8s_master_eip}:/home/ubuntu/cloudcart/deploy-sock-shop/Monitoring
   EOT
